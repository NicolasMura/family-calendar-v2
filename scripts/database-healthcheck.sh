...
if mongo "$host:$DB_PORT/$MONGODB_DB_MAIN" -u $MONGO_INITDB_DBUSER_USERNAME -p $MONGO_INITDB_DBUSER_PASSWORD --eval 'quit(db.getCollection("users").find().count() ? 0 : 2)'; then
